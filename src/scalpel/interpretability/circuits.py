...
def isolate_bias_circuit(
    ...
    """Rank all components by their write onto the bias direction and select
    ...
    for layer in range(lm.num_layers):
        ...
        head_effect = ((writes_a - writes_b) @ v).mean(dim=1)  # [H]
        for head in range(lm.num_heads):
            raw.append(("attn_head", layer, head, head_effect[head].item()))

        mlp_effect = ((acts_a.mlp_out[layer] - acts_b.mlp_out[layer]) @ v).mean().item()
